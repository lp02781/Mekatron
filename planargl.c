  ...
  if (gerak == 1) {
    ...
    while (k <= N*PERIOD) {
      ...
      if (counter%((N/divider)) == 0) {
        convert3();
        printf("%.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f\n",xx,yy,z, (q1*RTD)-90, q2*RTD,q3*RTD,q4*RTD,((q1*RTD)-(90)), fmod(((q1*RTD)-(90)), 360));
...
//         XOR = sudut1 ^ sudut2 ^ sudut3 ^ sudut4;
        
//         write(fd,&header,sizeof(header));//header
//         write(fd,&XOR,sizeof(XOR));//data XOR
//         write(fd,&sudut1,sizeof(sudut1));//data sudut 1
//         write(fd,&sudut2,sizeof(sudut2));//data sudut 2
//         write(fd,&sudut3,sizeof(sudut3));//data sudut 2
...
//        if (sudut3 < 0) sudut3 += 360;
       ...
       XOR = kirimsudut1_0 ^ kirimsudut1_a ^ kirimsudut2_0 ^ kirimsudut2_a ^ kirimsudut3_0 ^ kirimsudut3_a ^ sudut4;
        ...
        write(fd,&kirimsudut3_0,sizeof(kirimsudut3_0));//data sudut 3
        write(fd,&kirimsudut3_a,sizeof(kirimsudut3_a));//data sudut 3
        // write(fd,&kirimsudut3_b,sizeof(kirimsudut3_b));//data sudut 3
        write(fd,&sudut4,sizeof(sudut4));//data sudut 4
...
void turunin_z(){
  x_final = 0.1361; y_final = 0.0069; z_final = 0.0648;
  if (gerak == 1) {
    convert();
    trajectory_init();
    q4 = 30*DTR*RTD;
...
void ambil_objek(){
  ...
  printf("%.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f\n",xx,yy,z, (q1*RTD)-90, q2*RTD,q3*RTD,q4,((q1*RTD)-(90)), fmod(((q1*RTD)-(90)), 360));
  ...
  write(fd,&kirimsudut3_a,sizeof(kirimsudut3_a));//data sudut 3
  // write(fd,&kirimsudut3_b,sizeof(kirimsudut3_b));//data sudut 3
  write(fd,&q4,sizeof(q4));//data sudut 4
  write(fd,&XOR,sizeof(XOR));
...
void naikin_z(){
  x_final = 0.1361; y_final = 0.0069; z_final = 0.1311;
  if (gerak == 1) {
    convert();
    trajectory_init();
    q4 = 30*DTR*RTD;
...
void gerak_letak(){
  x_final = 0.1172; y_final = 0.1459; z_final = 0.0649;
  if (gerak == 1) {
    convert();
    trajectory_init();
    //printf("init y=%.2f z=%.2f\n", y_init, z_init);
...
void keyboard(unsigned char key, int i, int j)
{
   ...
   case '8' : x_final = 0.1430; y_final = 0.0039; z_final = 0.1311; q4=0*DTR; gerak = 1; break;

	case 'w' : objTrans += 0.001; break;
	case 'W' : objTrans -= 0.001; break;
 	//Kamera	
